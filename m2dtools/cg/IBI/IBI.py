...
def lj(r, epsilon1, epsilon2, sigma, slope=1):
    e_pot = 4 * (epsilon1 * (sigma / r) ** 12 - epsilon2 * (sigma / r) ** 6)
    # linear extrapolation using the slope at sigma, 0
    idx = np.where(e_pot < 0)[0][0]
    # e_pot[:idx] = e_pot[idx] + slope * (r[:idx] - r[idx])
    softening_factor = np.exp(-r[:idx]*slope)
    e_pot[:idx] = e_pot[:idx]*softening_factor
    return e_pot
