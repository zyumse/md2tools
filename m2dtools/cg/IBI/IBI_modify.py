...
def rscale_table_pot(r, e, scale):
    ...
    r_scaled = (r - r[-1]) * scale + r[-1]  # scale from the tail
    # corresponding e to the original r in the scaled r
    e_scaled = np.interp(r, r_scaled, e)
    e_scaled -= e_scaled[-1]
...
class IBISimulation:
    ...
    def initialize_parameters(self):
        ...
        for i, keyname in enumerate(self.pair_types):
            ...
            if self.config["use_existing_FF"]:
                ...
                print(f"Read existing FF for {keyname}", flush=True)
        ...
        if len(self.bond_types) > 0:
            ...
            for i,keyname in enumerate(self.bond_types):
                if self.config["use_existing_bondFF"]:
                    self.e_bond[f"bond_{keyname}"] = tl_lmp.read_table_pot(self.config["use_existing_bondFF"], f"bond_{keyname}")[1]
                else:
        ...
        if len(self.angle_types) > 0:
            ...
            for i, keyname in enumerate(self.angle_types):
                if self.config["use_existing_bondFF"]:
                    self.e_angle[f"angle_{keyname}"] = tl_lmp.read_table_pot(self.config["use_existing_bondFF"], f"angle_{keyname}")[1]
                else:
    ...
    def update_potentials(self):
        ...
        for key in self.property["RDF"].keys():
            ...
            error_PDF[key] = np.log((self.property["RDF"][key] + 1e-8)/(self.pdf_ref[key] + 1e-8)) * new_weight
        ...
        if 'bl' in self.property.keys():
            ...
            for key in self.property["bl"].keys():
                ...
                if self.config['smooth_bond'] and self.i_iter % self.config["smooth_freq"] == 0:
                    ...
                    # self.e_bond[key] = UnivariateSpline(self.r_bond, self.e_bond[key], s=self.config['smooth_s'], k=3)(self.r_bond)
        ...
        if "angle" in self.property.keys():
            ...
            for key in self.property["angle"].keys():
                ...
                if self.config['smooth_angle'] and self.i_iter % self.config["smooth_freq"] == 0:
                    ...
                    # self.e_angle[key] = UnivariateSpline(self.r_angle, self.e_angle[key], s=self.config['smooth_s'], k=3)(self.r_angle)
