...
class IBISimulation:
    def __init__(self, config_path, n_cpus):
    ...
    def update_potentials(self):
        ...
        for key in self.property["RDF"].keys():
            ...
            self.e_pot[key] = self._smooth_potential(self.r_pot, self.e_pot[key])[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dir", type=str, default="./", help="directory to run")
    parser.add_argument("--n_cpus", type=int, default=1, help="Number of CPUs to use")
    args = parser.parse_args()
    if args.config:
        config_file = args.config
    simulation = IBISimulation(config_file, args.n_cpus)
    print(simulation.n_cpus, simulation.n_iter)
    simulation.directory = args.dir
    simulation.process_results_bonded()
